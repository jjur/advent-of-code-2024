...
def solve(input, target):
    ...
    concat_part = int(str(input[0]) + str(input[1]))
    new_input = input[2:]
    return solve([sum_part] + new_input, target) or solve([multiply_part] + new_input, target) or solve([concat_part] + new_input, target)
